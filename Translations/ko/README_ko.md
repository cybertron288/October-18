...
**October-18**을 어떤 방법으로 설치하든지 간에 **시스템 환경설정을 꼭 닫아주세요**

### 수동 :hand:

1. [다운로드하려면 여기를 클릭하세요](https://github.com/pedrommcarrasco/Brooklyn/releases/download/2.0.1/October18.saver.zip)
2. **October18.saver**을 여세요 (더블 클릭)
3. `'October18.saver'은(는) 확인되지 않은 개발자가 배포했기 때문에 열 수 없습니다.`라는 메시지가 표시되면, `승인`을 클릭하세요
4. `시스템 환경설정`을 여세요
...
1. 터미널을 실행하세요
2. `brew cask install october-18`을 입력하세요

## 제거 🗑️

### 수동 :hand:

- 시스템 환경설정에서 Brooklyn을 우클릭 하여 `Delete October-18`을 선택하거나,
- `/Library/Screen Savers`와 `/Users/USERNAME/Library/Screen Savers`에서 `October18.saver`를 삭제하세요.

### Homebrew 🍺

1. 터미널을 실행하세요
2. `brew cask uninstall october-18`을 입력하세요
